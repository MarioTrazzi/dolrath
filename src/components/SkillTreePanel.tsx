 ...
 *
 * Layout por classe (ver skillTreeLayouts): mago=espiral, guerreiro=espada (montante),
 * ladino=flecha de baixo p/ cima, monge=mandala em trança dupla. O canvas pode ser
 * COMPRIDO (retrato) — a página rola verticalmente; o hub nem sempre é o centro.
 *
 * Estados do nó: aprendido (ouro vivo + brilho) · disponível (borda ouro, anel pulsando
 * se dá pra pagar) · bloqueado (chumbo apagado). No mobile o detalhe do nó vira uma
 * sheet fixa no rodapé (o painel lateral só existe em lg+).
 */
...
export default function SkillTreePanel({ tree, paths, purchased, availablePoints, onSpend, busy, classId }: SkillTreePanelProps) {
  ...
  const hub = dims.hub;
  const pctX = (v: number) => `${(v / dims.w) * 100}%`;
  const pctY = (v: number) => `${(v / dims.h) * 100}%`;
  const showRings = layout === 'spiral' || layout === 'mandala';

  // Corpo do detalhe do nó — usado na coluna lateral (lg+) e na sheet mobile
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${FRAME}`, background: PANEL_BG }}>
      ...
      <div className="flex flex-col lg:flex-row">
        {/* ✦ ASTROLÁBIO — canvas com geometria por classe (pode ser retrato) */}
        <div className="flex-1 min-w-0 p-3 sm:p-4">
          <div
            ...
            style={{
              ...
              background:
                `radial-gradient(circle at ${(hub.x / dims.w) * 100}% ${(hub.y / dims.h) * 100}%, rgba(201,162,95,0.10) 0%, rgba(201,162,95,0.03) 24%, transparent 44%)`,
            }}
          >
            ...
            <svg viewBox={`0 0 ${dims.w} ${dims.h}`} className="absolute inset-0 h-full w-full" aria-hidden>
              {showRings &&
                [0.28, 0.52, 0.76, 0.98].map((f, i) => (
                  <circle key={i} cx={hub.x} cy={hub.y} r={HUB_R + (dims.w / 2 - HUB_R) * f}
                    fill="none" stroke="rgba(201,162,95,0.06)" strokeWidth={1} />
                ))}
              {layout === 'sword' && (
                <>
                  <line x1={hub.x} y1={90} x2={hub.x} y2={dims.h - 56} stroke="rgba(201,162,95,0.05)" strokeWidth={1} />
                  <line x1={44} y1={hub.y} x2={dims.w - 44} y2={hub.y} stroke="rgba(201,162,95,0.05)" strokeWidth={1} />
                </>
              )}
              {layout === 'arrow' && (
                <>
                  <line x1={hub.x} y1={hub.y} x2={hub.x} y2={hub.y - 700} stroke="rgba(201,162,95,0.06)" strokeWidth={1} />
                  <line x1={hub.x} y1={hub.y} x2={hub.x - 154} y2={hub.y - 630} stroke="rgba(201,162,95,0.05)" strokeWidth={1} />
                  <line x1={hub.x} y1={hub.y} x2={hub.x + 154} y2={hub.y - 630} stroke="rgba(201,162,95,0.05)" strokeWidth={1} />
                </>
              )}

              {placedByPath.map(({ path, placed, spokeFrom }) => {
                const segs: React.ReactNode[] = [];
                const spoke = spokeFrom === null ? null : (spokeFrom ?? hub);
                if (placed[0] && spoke) {
                  const p0 = placed[0];
                  const lit = owned.has(p0.node.id);
                  segs.push(
                    <line key={`${path.id}-spoke`} x1={spoke.x} y1={spoke.y} x2={p0.x} y2={p0.y}
            ...
            {/* Medalhão do hub (a "assinatura" — o núcleo de poder) */}
            <div
              className="absolute grid place-items-center rounded-full text-center"
              style={{
                left: pctX(hub.x), top: pctY(hub.y), transform: 'translate(-50%,-50%)',
                width: `${(HUB_R * 2 / dims.w) * 100}cqw`, height: `${(HUB_R * 2 / dims.w) * 100}cqw`,
            ...
            {/* Rótulos de caminho — posição calculada pelo layout */}
            {placedByPath.map(({ path, placed, label }) => {
              ...
              return (
                <div
                  ...
                  style={{
                    left: pctX(label.x), top: pctY(label.y),
        ...
        {/* 📜 Detalhe do nó — coluna lateral no desktop; sheet fixa no rodapé no mobile
            (canvas comprido rola, então o detalhe precisa acompanhar a viewport) */}
        <div className="hidden lg:block lg:w-72 lg:shrink-0 lg:border-l p-3 sm:p-4"
          style={{ borderColor: FRAME, background: 'rgba(0,0,0,0.22)' }}>
          {selected ? (
            <div className="lg:sticky lg:top-4">{detailBody}</div>
          ) : (
      ...
      {/* Sheet mobile — acompanha a viewport enquanto o canvas comprido rola */}
      {selected && (
        <div
          className="lg:hidden fixed inset-x-0 bottom-0 z-50 rounded-t-xl px-4 pt-3"
          style={{
            background: 'linear-gradient(180deg, rgba(32,32,36,0.98), rgba(24,24,27,0.99))',

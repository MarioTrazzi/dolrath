...
export default function SimpleDungeon({ characterId, character, onCharacterUpdate }: SimpleDungeonProps) {
  ...
  const attack = async () => {
    ...
    // Gerar monstro baseado no andar atual
  ...
  const handleCombatEnd = (victory: boolean, rewards?: any, finalHp?: number, finalMp?: number) => {
    ...
    if (victory) {
      setCombatLog(prev => [...prev, `🎉 Você derrotou o monstro!`])
      if (rewards) {
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            <div className="text-center">
              ...
              <div className="text-sm opacity-80">⚡ {STAMINA_COSTS.EXPLORE} stamina</div>
            </div>
          </button>
          ...
          >
            <div className="text-center">
              ...
              <div className="text-sm opacity-80">⚡ {STAMINA_COSTS.COMBAT} stamina</div>
            </div>
          </button>

...
interface TransferQuantityDialogProps {
  open: boolean;
  item: Item;
  /** Quantidade máxima disponível na pilha de origem. */
  maxQuantity: number;
  /** Para onde o item vai (define o texto/cor do botão). */
...
/**
 * 📦 Diálogo de quantidade ao transferir uma pilha entre inventários (drag & drop).
 * Só aparece quando a pilha tem mais de 1 item — permite escolher uma quantidade
 * (campo + slider) ou enviar tudo de uma vez.
 */
export default function TransferQuantityDialog({
  open,
  item,
  maxQuantity,
  destinationLabel,
  destinationAccent = '#3b82f6',
  onConfirm,
  onClose,
  ...
  return createPortal(
    ...
    >
      <div
        ...
        style={{ background: 'linear-gradient(180deg, #1c232b, #11161b)', borderColor: `${destinationAccent}66` }}
      ...
      >
        ...
        <div className="p-4 flex flex-col gap-3">
          ...
          <input
            ...
            onChange={(e) => setQty(clamp(Number(e.target.value)))}
            className="w-full"
            style={{ accentColor: destinationAccent }}
          />
        </div>

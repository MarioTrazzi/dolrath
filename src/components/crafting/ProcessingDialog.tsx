...
// ⚙️ Dialog de PROCESSAMENTO — Bancada de Beneficiamento no estilo BDO.
//
// Clone estrutural do ForgeDialog (primitivos de bdoTheme.tsx): o insumo
// processado fica no losango central e a matéria-prima crua em molduras num
// arco abaixo, ligadas por linhas-circuito. Diferença de regra: processamento
// é o modelo do REFINO — conversão SEM falha (chance 1), XP fixo da receita,
// gating por minLevel próprio (não pela tabela de raridade).
...
// Geometria do circuito (mesma do ForgeDialog): saída no alto, insumos num arco abaixo.
const BOX_W = 320;
const BOX_H = 252;
...
export default function ProcessingDialog({
  ...
  // Processamento não falha: o veredito, quando existe, é sempre sucesso.
    ...
    <>
      <BdoDialogShell open={open} onClose={onClose} icon="⚙" title="Processamento">
        ...
        {!recipe ? (
          ...
          <>
            {/* ✦ Circuito da bancada: insumos em arco → cometas → saída no losango */}
            <div className="relative px-5 pb-1 pt-4">
              ...
              <div className="relative mx-auto" style={{ width: BOX_W, height: BOX_H }}>
                ...
                {/* Sem falha / gating no canto (processar é conversão garantida) */}
                    ...
                    {phase === 'charging' ? (
                      ...
                      >
                        <ItemThumb
                          ...
                          emoji={processingItemEmoji(recipe.outputName)}
                          className="text-3xl"
                        />
                      </span>
                    )}
                    ...
                    >
                      ...
                      >
                        <span className={`block h-full w-full overflow-hidden rounded-[2px] ${enough ? '' : 'opacity-40 grayscale'}`}>
                          <ItemThumb name={m.name} emoji={processingItemEmoji(m.name)} className="text-xl" />
                        </span>
                        <span
            ...
            >
              <div className="sticky top-0 z-10 flex items-center justify-between border-b border-black/70 bg-gradient-to-b from-[#2b2b2f] to-[#1a1a1d] px-4 py-2.5">
                <h3 className="flex items-center gap-2 text-[15px] font-semibold tracking-wide text-[#dcdce0]">
                  <span style={{ color: GOLD }}>📖</span> Livro de Processamento
                </h3>
                <button
                  onClick={() => setBookOpen(false)}

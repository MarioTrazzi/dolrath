...
import { getItemMarketContractAddress, getItemMarketProvider } from './itemMarketOnchain'
...
function readRetryConfig() {
  const retriesRaw = Number(process.env.ITEM_MARKET_RECEIPT_RETRIES ?? process.env.GOLD_PAYMENT_RECEIPT_RETRIES ?? 6)
  const delayMsRaw = Number(process.env.ITEM_MARKET_RECEIPT_DELAY_MS ?? process.env.GOLD_PAYMENT_RECEIPT_DELAY_MS ?? 1000)
...
export async function verifyListingPurchasedTx(params: {
  txHash: string
  expectedBuyer?: string
  expectedListingId?: bigint
  contractAddress?: string
}) {
  const contractAddress = (params.contractAddress || getItemMarketContractAddress()).trim()
  if (!contractAddress) throw new Error('Missing ITEM_MARKET_CONTRACT_ADDRESS')

  const provider = getItemMarketProvider()
  ...
  return {
    ...
    priceGold: parsed.args.priceGold as bigint,
  }
}
